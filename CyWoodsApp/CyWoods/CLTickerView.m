...
//        NSArray* gestureRecognizers = [self.scrollview gestureRecognizers];
//        for (UIGestureRecognizer* recog in gestureRecognizers) {
//            if ( [recog isKindOfClass:[UIPanGestureRecognizer class]] )
//                [recog removeTarget:self.scrollview.delegate action:@selector(scrollViewDidScroll:)];
//        }
       ...
       // self.userInteractionEnabled = NO;
        ...
        label.textColor =  [UIColor colorWithRed:255/255.0f green:204/255.0f blue:0/255.0f alpha:1.0f];
...
- (void)startScrolling
    {
        if (!startScrolling)
        {
            startScrolling = YES; scrollingTimer = [NSTimer scheduledTimerWithTimeInterval:SCROLLING_TIME_INTERVAL target:self selector:@selector(scroll:) userInfo:nil repeats:YES];
