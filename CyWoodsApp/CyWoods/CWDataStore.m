...
-(void)refresh:(NSMutableDictionary *)dict completion:(void (^)(id ret)) block{
    ...
    failure:^(AFHTTPRequestOperation *operation, NSError *error)
    {
        ...
        [[[UIAlertView alloc] initWithTitle:@"Error" message:[NSString stringWithFormat:@"Oh no! There was an error refreshing data: %@ It probably isn't my fault so please don't hurt me!", error.localizedDescription] delegate:nil cancelButtonTitle:@"Go Away!" otherButtonTitles:nil] show];
        if( block )
            block(nil);
        [(CWAppDelegate *)[UIApplication sharedApplication].delegate stopLoading];
    }];
...
-(NSDictionary *)teacherInfo:(NSString *)name
{
    ...
    return [NSDictionary dictionaryWithObjectsAndKeys:@"Information for this teacher is not available yet", @"description", name, @"name", nil];
}
...
-(void)addToNotifer:(NSString *)name :(NSString *)grade :(Boolean *)threshold
{
   ...
   if(threshold == true)
   {
       value = [NSString stringWithFormat:@"Lower %@", grade];
   }
   else
   {
       value = [NSString stringWithFormat:@"Higher %@", grade];
   }
    
   [_notes setValue:value forKey:name];
    NSUserDefaults *defaults = [NSUserDefaults standardUserDefaults];
    [defaults setObject:_notes forKey:@"Notifiers"];
...
{
    NSUserDefaults *defaults = [NSUserDefaults standardUserDefaults];
    NSMutableDictionary *results = [defaults objectForKey:@"Notifiers"];
    [[[UIAlertView alloc] initWithTitle:@"Warning!" message:[NSString stringWithFormat:@"Data %@", results]delegate:nil cancelButtonTitle:@"Leave me alone!" otherButtonTitles:nil] show];
    NSLog(@"%@",results);
}
